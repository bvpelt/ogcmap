...
export class BrtlayerService {
  ...
  async createBrtLayer(): Promise<LayerInfo | null> {
    try {
      // Get tile matrix set
      ...
      if (tileMatrixSet) {
        ...
        console.log("BRT layer created successfully");

        // Apply Mapbox style after layer is created
        await this.applyMapboxStyleToLayer(brtLayer, this.styleUrl);
